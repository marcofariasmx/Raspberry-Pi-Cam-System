...
# Configure logging to filter repetitive requests
import logging

class EndpointFilter(logging.Filter):
    def filter(self, record):
        # Uvicorn access logs use record.args[2] for the endpoint path
        if record.args and len(record.args) >= 3:
            endpoint = record.args[2]
            return endpoint not in ["/health", "/api/camera/metrics"]
        return True
